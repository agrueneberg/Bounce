(function () {
    ...
    validatePermissions = function (permissions) {
        var fieldsValid, operators, operatorsValid;
     // Validate fields.
        fieldsValid = Object.keys(permissions).every(function (field) {
            return ["govern", "read", "write", "add", "_inherit"].indexOf(field) !== -1;
        });
        if (fieldsValid === false) {
            return false;
        } else {
         // Validate operators.
            operators = Object.keys(permissions).filter(function (operator) {
                return ["govern", "read", "write", "add"].indexOf(operator) !== -1;
            });
            operatorsValid = operators.every(function (operator) {
                if (Array.isArray(permissions[operator]) === false) {
                    return false;
                } else {
                    return permissions[operator].every(function (arg) {
                        return (((arg.hasOwnProperty("username") === true && typeof arg.username === "string") && !(arg.hasOwnProperty("role") === true && ["authenticated", "public"].indexOf(arg.role) !== -1)) || (arg.hasOwnProperty("role") === true && ["authenticated", "public"].indexOf(arg.role) !== -1) && !(arg.hasOwnProperty("username") === true && typeof arg.username === "string")) && (arg.hasOwnProperty("state") === true && ["all", "self", "none"].indexOf(arg.state) !== -1);
                    });
                }
            });
            if (operatorsValid === false) {
                return false;
            } else {
             // Validate link.
                if (permissions.hasOwnProperty("_inherit") === true && typeof permissions._inherit === "string") {
                    return true;
                } else {
                    return operators.length === 4;
                }
            }
        }
    };
    ...
    module.exports = function (options) {
        ...
        hasPermission = function (permission, permissions, creator, user, callback) {
            async.whilst(function () {
                ...
                if (permissions.hasOwnProperty(permission) === false && permissions.hasOwnProperty("_inherit") === true && typeof permissions._inherit === "string") {
                ...
                if (err !== undefined) {
                    ...
                    operator = permissions[permission];
                 // Strategy 1: explicit access.
                    args = operator.filter(function (arg) {
                        return arg.hasOwnProperty("username") === true && arg.username === getUserId(user);
                    });
                    if (args.length > 0 && (args[0].state === "all" || (args[0].state === "self" && getUserId(user) === creator))) {
                        callback(null, true);
                    } else {
                     // Strategy 2: authenticated access.
                        args = operator.filter(function (arg) {
                            return arg.hasOwnProperty("role") === true && arg.role === "authenticated";
                        });
                        if (getUserId(user) !== "public" && args.length > 0 && (args[0].state === "all" || (args[0].state === "self" && getUserId(user) === creator))) {
                            callback(null, true);
                        } else {
                         // Strategy 3: public access.
                            args = operator.filter(function (arg) {
                                return arg.hasOwnProperty("role") === true && arg.role === "public";
                            });
                            if (args.length > 0 && (args[0].state === "all" || args[0].state === "self")) {
                                callback(null, true);
                            } else {
                                callback(null, false);
                            }

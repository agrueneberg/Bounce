        ...
        register = function (user, callback) {
            if (user.hasOwnProperty("username") === false || user.hasOwnProperty("password") === false) {
                ...
                if (user.username === "public") {
                    callback(new errors.Conflict("The username \"public\" is not allowed."));
                } else {
                    dataSource.then(function (db) {
                        db.collection("ming.users", function (err, collection) {
                            if (err !== null) {
        ...
        hasPermission = function (permission, permissions, creator, user, callback) {
            async.whilst(function () {
                ...
                if (err !== undefined) {
                    ...
                    if (permissions[permission].hasOwnProperty(getUserId(user)) === false) {
                        callback(new errors.Forbidden());
                    } else {
                        if (permissions[permission][getUserId(user)] === "all" || permissions[permission][getUserId(user)] === true) {
                            callback(null, true);
                        } else if (permissions[permission][getUserId(user)] === "self" && getUserId(user) === creator) {
                            callback(null, true);
                        } else {
                            callback(null, false);
                        }
